...
class FillProcess:
    ...
    def FillProcess(self, source_image, ref_image, source_mask, ref_mask, iterations):
        ...
        mask_diptych = mask_diptych[:, :, 0]
        diptych_ref_tar = torch.from_numpy(diptych_ref_tar).unsqueeze(0).float() / 255.0
        mask_diptych = torch.from_numpy(mask_diptych).unsqueeze(0).float() / 255.0



        return (diptych_ref_tar, mask_diptych, H1, W1, H2, W2, old_tar_image, tar_box_yyxx_crop, show_diptych_ref_tar)
